...
// متغير رابط الـ Worker المركزي ومعرف التطبيق (قابل للتخصيص عبر env أو القيم الافتراضية)
export const WORKER_SYNC_URL = (
  import.meta.env?.VITE_CLOUDFLARE_WORKER_URL 
    ? `${import.meta.env.VITE_CLOUDFLARE_WORKER_URL.replace(/\/+$/, '')}/api/v1/user/sync`
    : "https://rooh-network-api.workers.dev/api/v1/user/sync"
);

export const CURRENT_APP_ID = import.meta.env?.VITE_ROOH_APP_ID || "rooh_app_name"; // يتم تغييره حسب المنصة (مثلاً: rooh_voice, rooh_games, rooh_pro)

/**
 * دالة تسجيل الدخول الموحدة بـ Google مع إظهار اختيار الإيميلات والمزامنة
 */
export const handleGoogleLogin = async (onSuccessCallback, onErrorCallback) => {
  try {
